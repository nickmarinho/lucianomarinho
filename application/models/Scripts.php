...
class Model_Scripts
{
	...
	*/
	public function getTable()
	{
		...
		if(null === $this->_table)
		{
			...
			*/
			require_once APP_PATH . '/models/DbTable/Scripts.php';
			$this->_table = new Model_DbTable_Scripts();
		}
	...
	* @return Zend_Db_Table_Rowset_Abstract
	*/
	public function fetchEntriesFront($options=null)
	{
		$table = $this->getTable();
		$select = $table
			->select()
			->setIntegrityCheck(false)
			->from($table, array('scripts_categories.name as category',
								'scripts.name as scriptname',
								'scripts.display',
								'scripts.title',
								'scripts.meta_keywords',
								'scripts.meta_description',
								'scripts.content'))
			->join('scripts_categories', 'scripts_categories.id = scripts.category_id');
			
		if(count($options) > 0)
		{
			$where = "1 ";
			foreach($options as $key => $value){ $where .= " AND scripts." . $key . " LIKE '%" . $value . "%' "; }
			$select->where($where);
		}
