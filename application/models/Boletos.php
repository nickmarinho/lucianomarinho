...
class Model_Boletos
{
	...
	*/
	public function getTable()
	{
		...
		if(null === $this->_table)
		{
			...
			*/
			require_once APP_PATH . '/models/DbTable/Boletos.php';
			$this->_table = new Model_DbTable_Boletos();
		}
	...
	*/
	public function fetchEntryByType($url)
	{
		$table = $this->getTable();
		foreach($url as $key => $value){ $select = $table->select()->where($key . ' = ?', $value); }
		return $table->fetchAll($select)->toArray();
	}
	...
	*/
	...
	public function fetchLastNumero()
	{
		...
		$select = $table
			->select()
			->setIntegrityCheck(false)
			->from($table, array('numero'));
		
		$select->where('id_cliente = ?', $id_cliente)
				->order('numero DESC')
				->limit(1);
		if($table->fetchRow($select) > 0) return $table->fetchRow($select)->toArray();
		else return '0';
	}
}
?>
